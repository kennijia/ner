...
def k_fold_run():
    ...
    for train_index, dev_index in kf_data:
        ...
        test_loss, f1 = run(word_train, label_train, word_dev, label_dev, vocab, device, writer, kf_index)
...
def simple_run():
    ...
    writer = SummaryWriter(log_dir=config.exp_dir + "/tensorboard")
    run(word_train, label_train, word_dev, label_dev, vocab, device, writer)
    writer.close()


def run(word_train, label_train, word_dev, label_dev, vocab, device, writer, kf_index=0):
    # build dataset
    ...
    # train and test
    train(train_loader, dev_loader, vocab, model, optimizer, scheduler, device, writer, kf_index)
    with torch.no_grad():
